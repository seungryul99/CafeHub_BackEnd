...
public class CafeService {
    ...
    public ResponseDTO<CafeInfoResponseDTO> getCafeInfo(Long cafeId) {
        ...
        List<CafeInfoResponseDTO.BestReviewDetail> topNReview = reviewRepository.findTopNReviewsByCafeId(cafeId, TOP_N_REVIEW_SIZE);
    ...
    private void updateReviewPhotoUrls(List<CafeInfoResponseDTO.BestReviewDetail> topNReview){

        // Top N 리뷰의 ID 리스트 만들기
        List<Long> reviewIds = topNReview.stream()
                .map(CafeInfoResponseDTO.BestReviewDetail::getReviewId)
                .toList();
        ...
        for (CafeInfoResponseDTO.BestReviewDetail rd : topNReview){
        ...
        for (CafeInfoResponseDTO.BestReviewDetail x : topNReview) {
            x.setPhotoUrls(reviewPhotosMap.get(x.getReviewId()));
        }
    }

    private void updateLikeStatusInReviews(List<CafeInfoResponseDTO.BestReviewDetail> topNReview, Long loginMemberId) {
